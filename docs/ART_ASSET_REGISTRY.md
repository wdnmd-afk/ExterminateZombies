...
| 玩家/人物 | Kenney Topdown Shooter | 已接入五名角色 | `Hitman 1`、`Soldier 1`、`Man Blue`、`Man Brown` 的 `*_stand.png` 分别用于鹰眼、堡垒、疾行者、破阵者的躯干（守望者已换成自生成精灵）；同包 `*_gun.png` 派生出持枪手层（见下一行）；同包 `Vector/vector_characters.svg` 的对应 `hold` 姿态切出五张战前档案立绘；其余人物、僵尸、瓦片与对象仍为备选。**躯干不能退回 `*_hold.png`**：该姿态双手张开、手里没有东西，武器会从两手之间穿过 | `PreloadScene`、`PreparationScene`、`GameAssetManager`、`Player`、`process_character_assets.py`、`process_character_hand_layers.py` | `src/assets/downloaded/characters/kenney-topdown-shooter/` | [OpenGameArt](https://opengameart.org/content/topdown-shooter) | [原始 ZIP](https://opengameart.org/sites/default/files/topdown-shooter.zip) | CC0 1.0 |
| 持枪手层 | Kenney Topdown Shooter 位图派生 | 已接入四名角色 | 从 `*_gun.png` 减去躯干层与 `weapon_gun.png` 抽出的前臂与拳头，叠在武器**之上**压住握把。画幅与躯干贴图共用几何中心，运行时同 origin、同缩放、同旋转角。守望者的自生成精灵自带双拳，不用这一层 | `PreloadScene`、`GameAssetManager`、`Player` | `src/assets/processed/characters/hand-*.png` | 派生自上一行 Kenney 来源 | 由 `scripts/process_character_hand_layers.py` 生成 | CC0 1.0（继承 Kenney 原始许可） |
...
| 守望者实机精灵 | 项目内 AI 生成 | 已接入 | 守望者的关卡内实机躯干，`48 x 48` 正俯视朝右、自带握拳双手，不叠持枪手层。2026-08-21 重做：旧图是斜视图（能看到脸、腿与整只靴子），按瞄准角旋转时读作「躺平的身体在打转」。新图经 `inspect_character_candidates.py` 的机位/体型/拳心量化门控放行（v01 球体、v02 退回斜视均被拦下） | `PreloadScene`、`GameAssetManager`、`Player` | `src/assets/processed/characters/sprite-watcher.png` | 项目内生成，原图与生成环境见 `src/assets/generated/characters/SOURCE.md` | 由 `scripts/generate_character_assets.mjs` 生成、`scripts/process_character_assets.py sprite watcher` 处理 | 项目内生成资产（`gpt-image-2`，提示词已排除现实徽标与版权角色特征） |
...
| 基础感染体（项目生成） | Bomber 自生成方向表与图鉴立绘 | 已接入 | `bomber` 的四方向 `2048×2048` 移动表（单帧 `512`）与 `512×512` 图鉴立绘 | 全部战斗模式与怪物图鉴；`PreloadScene`、`GameAssetManager`、`Zombie`、`MonsterLibraryScene` | 源图 `src/assets/generated/zombies/Bomber_*.png`；产物 `src/assets/processed/zombies/bomber-directional-custom.png`、`bomber-portrait.png` | 无外部来源，本地图片代理按 `docs/design/ZOMBIE_PROMPTS.md` 6.4 生成，流程见 `docs/execution/2026-08-20-bomber-art-resource-rework.md` | 不适用 | 项目自有 |
| 基础感染体（项目生成） | Drifter 自生成方向表与图鉴立绘 | 已接入 | `drifter` 的四方向 `2048×2048` 移动表（单帧 `512`）与 `512×512` 图鉴立绘 | 全部战斗模式与怪物图鉴；`PreloadScene`、`GameAssetManager`、`Zombie`、`MonsterLibraryScene` | 源图 `src/assets/generated/zombies/Drifter_*.png`；产物 `src/assets/processed/zombies/drifter-directional-custom.png`、`drifter-portrait.png` | 无外部来源，本地图片代理按 `docs/design/ZOMBIE_PROMPTS.md` 6.6 生成，流程见 `docs/execution/2026-08-20-drifter-art-resource-rework.md` | 不适用 | 项目自有 |
...
| `src/assets/processed/zombies/drifter-directional-custom.png`、`drifter-portrait.png` | 项目生成源图 `src/assets/generated/zombies/Drifter_*.png`（洋红键控底） | 同上管线，`scripts/*_zombie_*` 三件套按 id 取 `scripts/zombie_asset_specs.json` 配置；采用版本 `v02` | `drifter`；`PreloadScene`、`GameAssetManager`、`Zombie`、`MonsterLibraryScene` | 惨白皮肤的阴影必须是冷蓝灰而非淡紫：淡紫会命中洋红键控判据被抠成透明洞，而惨白尸体的阴影在美术上最常见的画法恰好就是淡紫，属高发风险。垂落绷带须贴身且与手臂相连，飘成长条会被算成额外连通域并撑大共用缩放框 |
...
| --- | --- | --- | --- | --- | --- |
| Minigun - User Interface and rollcage barrel | 仅作型号参考 | 加特林的枪管束造型参考。2026-08-22 重画侧视图标后，产物**不再包含本素材的任何像素**；归档保留，署名义务不变 | `src/assets/downloaded/weapons/tiamalt-minigun/` | [OpenGameArt](https://opengameart.org/content/minigun-user-interface-and-rollcage-barrel) | CC-BY 3.0，作者 Tiamalt |
...
| `gatling.png` / `golden_m249.png` / `flamethrower.png` | 已接入 | HUD 槽位、拾取物、战前整备与武器图鉴的**侧视图标**（实机改用俯视图，见下一行） | `src/assets/processed/weapons/` | `scripts/process_heavy_weapon_profiles.py`（项目自绘，画幅锁定 132x48） | 项目自绘 |
