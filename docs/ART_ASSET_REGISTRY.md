...
| `src/assets/processed/zombies/runner-directional-custom.png`、`runner-portrait.png` | 项目生成源图 `src/assets/generated/zombies/Runner_*.png`（洋红键控底） | `scripts/generate_runner_assets.mjs` 生成候选，`scripts/process_runner_assets.py` 键控抠图、2×2 切帧、右向镜像、尺寸与基线归一、方向表组装 | `runner`；`PreloadScene`、`GameAssetManager`、`Zombie`、`MonsterLibraryScene` | 生成源图与两个脚本必须同时保留；候选检视用 `scripts/inspect_runner_candidates.py`。右向帧由左向镜像得到，不单独生成 |
