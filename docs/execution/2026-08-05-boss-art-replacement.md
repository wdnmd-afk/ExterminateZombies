...
## 0.4 毁灭爆破者动作接入（2026-08-10）

1. Kliver 的 `kliver-attack.png` 与 `kliver-death.png` 已进入运行时，分别用于毁灭爆破者的轰炸/震荡前摇和延迟死亡结算。
2. Scorpion 与 Gargant Boss 的攻击、受伤和死亡帧仍只归档，等待对应机制切片接入。
3. 具体阶段、能力、风险和验收清单见 `docs/execution/2026-08-10-bomber-boss-vertical-slice.md`。

## 0.5 猎杀者与母体动作接入（2026-08-10）

1. Scorpion 的 8 帧攻击与两张共 16 帧死亡图已接入猩红猎杀者；Gargant 的 5 帧攻击与两张共 16 帧死亡图已接入腐化母体。
2. `ZombieActionTextureLayout` 已支持一个动作按顺序拼接多张纹理源，实体层继续只读取单一动作动画键和总时长。
3. 两名 Boss 同步接入第二阶段与第二技能；代码和资源调用链已完成 V0 审阅，浏览器实景、完整关卡试玩与真人手感仍待验。
