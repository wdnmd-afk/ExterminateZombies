...
| V3 | 浏览器冷启动 + 纹理链路 | **通过**（见 §10.3.1） |
| V4 | Agent 实景：显示列表 + 画面像素 + 其余关卡回归 | **通过**（见 §10.3.2、§10.3.3） |
| V5 | 第二关完整试玩（波次/Boss/结算闭环） | **未执行** |
| V6 | 真人目视（拼贴感、接缝、对比度） | **未执行** |

结论等级按 `TESTING_RULES.md` §13：**客观通过，主观待验**。

探针脚本：`scripts/cdp_verify_battlefield_bitmap.mjs`（CDP 实景）、
`scripts/analyze_battlefield_screenshot.py`（截图像素判据）。
证据：`.debug-g52-bitmap/probe.json`、`.debug-g52-bitmap/level2.png`。

#### 10.3.1 V3 纹理链路：通过

三张纹理均在 TextureManager 内，且**不是** `__MISSING` 占位图，真实来源尺寸与
`BATTLEFIELD_TILE_SETS` 登记值逐项一致：

| 纹理键 | exists | 真实尺寸 | 占位图 |
| --- | --- | --- | --- |
| `env-battlefield-level2-ground` | true | 32×32 | false |
| `env-battlefield-level2-rail` | true | 1280×116 | false |
| `env-battlefield-level2-boundary` | true | 1280×20 | false |

无 console error，无 battlefield/texture 相关告警。帧推进已按 `TESTING_RULES` §9.3.1 确认
（6 → 46），排除后台节流导致的假加载失败。

#### 10.3.2 V4 显示列表与几何：通过

`DEPTH.ground` 层共 71 个对象，其中位图部分正是渲染分支应产出的 3 个 `TileSprite` + 4 个 `Image`：

| 对象 | 纹理 | 位置 | 尺寸 | 对应 §3.1 约束 |
| --- | --- | --- | --- | --- |
| TileSprite | 地面（Phaser 内部 canvas key） | (0,0) | 1280×720 | 铺满画布 |
| TileSprite | 左边界 | (0,0) | 20×720 | 厚度 20 ✓ |
| TileSprite | 右边界 | (1260,0) | 20×720 | 贴右缘 ✓ |
| Image | `...-rail` | (0,**92**) | 1280×116 | 上轨道带 y=92..208 ✓ |
| Image | `...-rail` | (0,**512**) | 1280×116 | 下轨道带 y=512..628 ✓ |
| Image | `...-boundary` | (0,0) | 1280×20 | 顶边界 ✓ |
| Image | `...-boundary` | (0,**700**) | 1280×20 | 底边界 ✓ |

#### 10.3.3 V4 画面像素：8/8 通过

这是唯一能证明"位图真的到了屏幕上"的证据。判据核心是**程序化纯色填充与位图瓦片的
区域内明度分布不同**：`fillRect` 区域的不同明度值恒为 1，位图必然多于 1。

| 取样区 | 均值 | 明度 | stdev | 不同明度值 |
| --- | --- | --- | --- | --- |
| 地面·左下开阔区 | `#2c2f31` | 46.3 | 1.37 | **22** |
| 地面·中央通道 | `#36342d` | 52.6 | 20.18 | 794 |
| 铁轨带·上 | `#4b5154` | 80.0 | **40.22** | 175 |
| 铁轨带·下 | `#2f363d` | 53.3 | **27.41** | 330 |
| 边界·顶 / 左 / 底 | `#1a1b1d` | 26.9 | 0.00 | 1 |

两条独立的交叉验证：

1. 地面实测 `#2c2f31` vs 派生脚本产出的 `#2b2e31`；边界实测 `#1a1b1d` vs 派生产出
   `#1a1b1d`（**完全一致**）。屏幕上的确实是本轮生成的那几张图。
2. 三条边界读到同一个值（极差 0.0），证明四边都铺上了同一张贴图，没有哪条漏铺。

边界 stdev 0.00 / distinct 1 不是缺陷：边界瓦片经 `strength=0.9` 归一后本就接近纯色，
它反倒给出了"程序化纯色是什么样"的对照——地面的 distinct 22 因此是有效判据。

#### 10.3.4 V4 其余关卡回归：通过

| 关卡 | ground 层类型 | 用位图键 |
| --- | --- | --- |
| level_1 | `Graphics:1, Sprite:12, Rectangle:25` | **false** |
| level_3 | `Graphics:1, Sprite:12, Rectangle:53` | **false** |
| level_2 | `Graphics:1, **TileSprite:3, Image:4**, Sprite:12, Rectangle:51` | **true** |

其余关卡无 `TileSprite`/`Image`、不含任何 `env-battlefield-*` 键，仍走程序化分支。
§2.2「不改其余九关外观」得到实景确认。

#### 10.3.5 三处判据/工具缺陷（均为本轮测量方法错误，非产品缺陷）

1. **页面内 `gl.readPixels` 全读到纯黑**（五个取样区 stdev 全 0、distinct 全 1）。
   原因是 Phaser 默认 `preserveDrawingBuffer: false`，帧呈现后 drawing buffer 即被清空，
   在帧回调之外读只能得到黑。改走 CDP `Page.captureScreenshot`（走合成器路径）后正常。
2. **截图坐标错配**。首版按"截图 == 逻辑画布 1280×720"取样，但实测截图 1264×625、
   横纵缩放 0.988/0.868 不一致，`y=650` 直接落到画布外黑边，被误读成"地面是纯色"。
   根因是宽屏侧栏把逻辑画布加宽到 **1520**（战场仍 1280×720 但起点在 x=120），
   而 `scale.gameSize.width` 报的是 1280、不可靠。改由缓冲反推
   （`renderScale = bufferHeight/720`，逻辑宽 = `bufferWidth/renderScale`）后对齐，
   战场原点算得 (101.1, 57.0)。分析脚本现在在横纵缩放不一致时**直接拒绝给结论**。
3. **判据量错了对象**。首版卡地面 `stdev > 1.5`，判成未通过——但地面本就选的是全部
   候选中最平坦那格（行/列方差 0.4/0.1），再经 0.842 降采样平滑，stdev 只有 1.37。
   能区分位图与纯色的是 **distinct**（纯色恒 1），不是 stdev。铁轨带同理：
   带高 116、钢轨居中，取样落在道砟段时与地面均值差仅 7，落在钢轨段则超 30，
   稳定判据是内部 stdev 而非区域均值差。

#### 10.3.6 探针设计缺陷：证据一度全部丢失

首版把 `writeFileSync` 放在脚本末尾，结果 level_1 回归步骤崩溃时，
前面已取得的纹理、显示列表、像素证据**全部丢失**——恰好是本轮唯一要的东西。
已改为每步 `persist()` 立刻落盘，并把回归段包进 `try`，让关停期异常记成一条独立结果
而不是让整个脚本失败。证据不能押在"脚本能跑到最后"这个前提上。
...
V3/V4 已完成（§10.3），以下是仍未做的：

1. **V5 第二关完整试玩**：三阶段 363 只敌人 + tank_boss + 结算闭环，确认掉落物、
   危险区预警、Boss 冲锋在新地面上的可读性。本轮只做了静态场景取样，没有战斗中的画面。
2. **V6 真人目视**：平铺接缝是否可见（§10.4 风险 2）、长时间视觉疲劳、
   与既有九关的风格一致性。Agent 不可替代。
3. **`npm run build`**：按 C-9 需单独申请。
4. **关停期既存缺陷**：见 `docs/execution/2026-09-01-shutdown-effect-pool-defect.md`。
   已归因为与 G5-2 无关（level_1/level_3 不走位图分支也同样崩），但**真实玩法是否受影响
   尚未确认**，需在修复前先判别。
...
| 脚本 | 用途 |
| --- | --- |
| `scripts/inspect_battlefield_tile_candidates.py` | 在无法目视图像时用"指标 + ASCII 轮廓 + 方向性方差"选瓦片 |
| `scripts/cdp_verify_battlefield_bitmap.mjs` | CDP 实景探针：纹理链路、显示列表、画布几何、回归 |
| `scripts/analyze_battlefield_screenshot.py` | 截图像素判据，含侧栏偏移换算与缩放错配拒绝 |

G5-5 扩展其余关卡时，前两个只需改主题 id，第三个需按各关的几何调整 `REGIONS`。
