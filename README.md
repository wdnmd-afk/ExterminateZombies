...
| 可用武器 | 8 把：沙漠之鹰初始配发，其余 7 把均通过敌人掉落获取 |
...
- 新局只配发沙漠之鹰；其余 7 把武器从对应感染体掉落中获得，数字键选择前四栏，鼠标滚轮可遍历全部已持有武器。
...
> 验证状态：仓库当前已经声明 `@types/node`，此前记录的缺依赖原因不再符合现状；最近一轮改动后尚未重新执行 `npm run typecheck` 与 `npm run build`，因此当前是否通过仍待命令验证。
...
- AK-47、Barrett M82、RPG-7 和 M79 已退出默认测试配发并进入正式掉落表；全部有限弹药武器都有补给来源，但首获节奏、弹药压力和数值仍待完整试玩平衡。
...
- `npm run typecheck` 与 `npm run build` 在最近一轮改动后尚未重新执行，当前结果待确认。
