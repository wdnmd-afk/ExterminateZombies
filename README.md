...
| 局内强化 | 24 张武器强化卡，覆盖全部 8 把武器；拾取强化包后四选一，同武器强化可叠加 |
...
> 验证状态：2026-08-14 最近一轮改动已通过 `npm test`（18 个文件、168 个用例）与 `npm run typecheck`；`npm run build` 和浏览器实景仍未执行。
...
- 战前整备、自适应弹药和 4×2 军械带的主实现已经落地，但尚未执行自动化、类型、浏览器布局和完整试玩验证；补给压力与单次数量仍待真实样本收敛。
- 武器强化系统已恢复普通感染体 3%-5%、Boss 100% 的正式配置概率，但 24 张卡的组合强度和单局出现节奏仍待完整试玩。
