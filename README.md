...
| 可用武器 | 8 把：首次战前整备从中选择 5 把组成出战编队，沙漠之鹰固定占第 1 槽；未选武器可通过战场拾取解锁许可 |
...
| 主动药品 | 绷带、急救、饮料；独立按键读条使用，饮料提供持续治疗与临时移速加成；三者各有独立 CC0 图标，HUD 药品槽与战场掉落物同图 |
...
- 固定关卡和无尽模式开局前进入战前整备；首次进入时先从 8 把武器中选满 5 把，再选择角色和首发武器，第一关仍固定教学手枪。
- 爆头由角色基础爆头率、武器修正和角色被动共同决定，在弹丸实际命中时逐目标独立判定；爆炸与环境伤害不能爆头。
- 首次整备选中的 5 把武器会获得军械许可；之后可在武器库从已解锁许可中维护 5 把出战编队。沙漠之鹰固定占第 1 槽，第一关仍以它作为当前武器。
...
- 武器库直接读取玩法配置与军械许可，除展示真实获取方式外还负责维护 5 槽出战编队；怪物图鉴展示敌人数值、档案介绍和处置建议。
...
| `ez:initialWeaponSelectionCompleted` | 是否已完成首次 8 选 5 武器编队 |
...
> `ERR_PACKAGE_PATH_NOT_EXPORTED: ./module-runner`；`npm run typecheck` 退出码 2，28 个错误全部是
> `letterSpacing` 不在 Phaser 3.80.1 的 `TextStyle` 类型里（`MainMenuScene`、`MonsterLibraryScene`、
> `WeaponLibraryScene`、`PreparationScene`），属仓库既有问题，非本轮改动引入。两项修复未在本轮范围内。
...
- 当前工作区的 `npm test` 与 `npm run typecheck` 都跑不出通过结论：前者被 `vitest 4` 与 `vite 5` 的依赖不兼容阻断，后者有 28 个仓库既有的 `letterSpacing` 类型错误（详见「验证与测试」）；生产构建同样未执行，不能声称生产包已验证。
