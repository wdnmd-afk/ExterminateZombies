...
| 可用武器 | 8 把：第一关固定手枪教学并交付 MP5；首次拾取永久解锁军械许可，第二关起可选择已解锁主武器 |
...
| 进度保存 | 版本化存档、关卡/军械许可、偏好主武器、无尽最佳波次、自定义键位和音量设置 |
...
- 第一关固定配发沙漠之鹰并在首阶段交付 MP5；第二关起从已解锁许可中选择一把主武器，与沙漠之鹰一同进入战斗。
- 起始沙漠之鹰拥有无限备用弹药，但仍保留弹匣和换弹节奏。
- RPG-7 与 M79 使用独立爆炸弹药，命中敌人、场景物、障碍或达到射程时触发一次范围爆炸，并保留玩家误伤与连锁规则。
- 敌人可掉落弹药、生命、地雷和武器；首次获得武器时自动装备、获得一个备用弹匣并永久解锁军械许可。
...
- HUD 显示生命、当前武器、弹匣、备用弹药、道具、波次和得分，并用固定 4×2 军械带展示全部已持有武器的可用状态。
...
> 验证状态：2026-08-17 最新基线已通过 `npm test`（21 个文件、188 个用例）、`npm run typecheck` 与 `npm run build`；Chrome 客观验收已覆盖主菜单、军械带、第一关、第二关、无尽首 Boss 和 G2-7 全部 24 张卡，正式强化流程已进入第二关结算，V6 真人手感/听感仍待完成。
...
- 战前整备、自适应弹药和 4×2 军械带已完成 V0-V5 客观验收；补给压力、切枪意愿和军械带认知负担仍需 V6 真人样本收敛。
- 武器强化系统已恢复普通感染体 3%-5%、Boss 100% 的正式配置概率；G2-7 已完成 24/24 张行为化和 V0-V5 客观验收，组合强度、单局出现节奏和 V6 主观体验尚未完成。
- 8 把武器共用 Kenney Survivor 的同一双手持枪姿态，尚无分武器持枪动画；武器表现只在 headless 浏览器验收过，斜向持枪贴合度与有头浏览器观感仍待确认。
- 十关原型已具备独立任务简报、主题化程序地面、边界、障碍、场景物和敌群配置；正式位图环境素材、逐关完整试玩和本轮最新浏览器截图验收仍缺失。
- `npm test` 与 `npm run typecheck` 已通过；生产构建、最新浏览器截图和完整流程验收仍待执行。
