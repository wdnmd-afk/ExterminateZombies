...
| 可用武器 | 11 把：首次战前整备从中选择 6 把组成出战编队，沙漠之鹰固定占第 1 槽；未选武器可通过战场拾取解锁许可 |
| 可玩角色 | 5 名：守望者、鹰眼、堡垒、疾行者、破阵者；全部初始开放，具备独立生命、移速、伤害、爆头率和专属被动 |
| 局内强化 | 33 张武器强化卡，覆盖全部 11 把武器（每把 3 张）；全部卡片均包含齐射、弹链、标记、击杀爆炸、子爆破、全自动或残留区等打法行为，拾取强化包后四选一，同武器强化可叠加 |
| 武器美术 | 11 把武器均使用对应真实型号的像素贴图；玩家使用自生成俯视精灵与武器覆盖层，枪口出弹点对齐枪管末端 |
| 武器库 | 11 项军械档案全部具备战斗配置，并从真实掉落表派生获取来源与概率 |
| 怪物图鉴 | 18 项感染体档案，包含 14 种普通/变异感染体与 4 个 Boss |
| 特殊敌人 | 带前摇的冲刺、远程投射，以及四个已接线的两阶段、双技能 Boss 机制战 |
| 战术元素 | 油桶、面粉桶、地雷、火焰残留区、阻敌粉尘区、连锁爆炸；三者都可从感染体掉落并由玩家携带布置 |
| 主动药品 | 绷带、急救、饮料；独立按键读条使用，饮料提供持续治疗与临时移速加成；三者各有独立 CC0 图标，HUD 药品槽与战场掉落物同图，并各有多个感染体掉落来源 |
| 音频 | 开放授权战斗/UI 音效、菜单/战斗循环、空间声像和三档音量设置 |
| UI 字体 | 阿里巴巴普惠体 3.0 55 Regular，统一用于全部运行时文字 |
| 进度保存 | 版本化存档、关卡/军械许可、六武器编队、偏好主武器、无尽最佳波次、自定义键位和音量设置 |
...
- 固定关卡和无尽模式开局前进入战前整备；首次进入时先从 11 把武器中选满 6 把，再选择角色和首发武器，第一关仍固定教学手枪。
- 爆头由角色基础爆头率、武器修正和角色被动共同决定，在弹丸实际命中时逐目标独立判定；爆炸与环境伤害不能爆头。
- 首次整备选中的 6 把武器会获得军械许可；之后可在武器库从已解锁许可中维护 6 把出战编队。沙漠之鹰固定占第 1 槽，第一关仍以它作为当前武器。
- 起始沙漠之鹰拥有无限备用弹药，但仍保留弹匣和换弹节奏。
- RPG-7 与 M79 使用独立爆炸弹药，命中敌人、场景物、障碍或达到射程时触发一次范围爆炸，并保留玩家误伤与连锁规则。
- 敌人可掉落弹药、药品、战术道具和武器；本局未满 6 把时首次获得的武器会加入当前编队并永久解锁许可，满编队时只解锁许可供后续在武器库调整。
...
- 每局初始携带绷带、急救和饮料，局内靠掉落补充：绷带来自杂兵与硬目标，急救只从坦克级和 Boss 掉，饮料来自高速感染体。读条期间可半速移动但不能开火、换弹、切枪或布置道具，同键可取消。战场上没有即时回血掉落，所有治疗都要付读条时间。
- 战术道具有地雷、油桶、面粉桶三种，都能从对应感染体掉落并由玩家携带布置：地雷近距自动引爆，两种桶放下后要自己补一枪，换来的是可选时机与残留区。
...
- HUD 显示生命、当前武器、弹匣、备用弹药、道具、关卡总进度和得分；单侧剩余逻辑宽度达到 120px 时启用宽屏侧栏并常驻展示 6 槽军械带，更窄时回退到战场内军械带，窗口与全屏变化会实时重排档位。
- HUD 会读取当前真实键位、显示换弹状态和 Boss 生命；设置页可调音量并清除战役、军械许可与无尽纪录。
- 武器库直接读取玩法配置与军械许可，除展示真实获取方式外还负责维护 6 槽出战编队；怪物图鉴展示敌人数值、档案介绍和处置建议。
...
| `ez:weaponLoadout` | 武器库保存的最多 6 把出战编队 |
...
- 原有主武器整备和自适应弹药已有既有客观验收；五角色、独立战前整备、六武器编队和固定侧栏 HUD 已完成当前工作区的命令与 Chrome 客观验证，真人可读性与手感验收仍待执行。
