...
| 可用武器 | 17 把：首次战前整备从中选择 6 把组成出战编队，沙漠之鹰固定占第 1 槽；未选武器通过关卡阶段奖励解锁许可 |
| 可玩角色 | 5 名：守望者、鹰眼、堡垒、疾行者、破阵者；全部初始开放，各有独立生命、移速、伤害、爆头率、专属被动与专属主动技能 |
| 角色技能 | 每名角色一个主动技能（默认 `E`）：压制脉冲、猎杀视界、装甲过载、相位疾冲、弹药过载；独立冷却，HUD 常驻显示冷却与生效窗口 |
| 局内强化 | 51 张武器强化卡，覆盖全部 17 把武器（每把 3 张）；每张卡都包含齐射、弹链、标记、击杀爆炸、子爆破、扇形几何或残留区等打法行为，拾取强化包后四选一，同武器强化可叠加 |
| 武器美术 | 17 把武器均有对应的像素侧视图标与俯视实机贴图；玩家使用自生成俯视精灵与武器覆盖层，枪口出弹点对齐枪管末端 |
| 武器库 | 17 项军械档案全部具备战斗配置，并从真实关卡奖励表派生解锁关卡与首次交付备弹 |
| 怪物图鉴 | 18 项感染体档案，包含 14 种普通/变异感染体与 4 个 Boss |
| 特殊敌人 | 带前摇的冲刺、远程投射，以及四个已接线的三阶段 Boss 机制战 |
| 战术元素 | 油桶、面粉桶、地雷、燃烧瓶、粉尘罐、高爆包、冷冻罐，以及火焰/粉尘/寒雾残留区和连锁爆炸 |
...
3. `第三关：封锁城区`：120 只分段敌群、两面可坍塌危墙与毁灭爆破者 Boss。
...
- 固定关卡和无尽模式开局前进入战前整备；首次进入时先从 17 把武器中选满 6 把，再选择角色和首发武器，第一关仍固定教学手枪。
- 每名角色有一个主动技能，默认 `E` 键释放，冷却 9~20 秒：守望者的压制脉冲清开包围并短暂无敌，鹰眼的猎杀视界把爆头率顶到上限并附加穿透，堡垒的装甲过载同时提升减伤、移速与伤害，疾行者的相位疾冲穿过敌群并在起点留下阻敌粉尘，破阵者的弹药过载在窗口期免除弹药消耗。
- 被动在战场上可见：角色脚下的指示环只在被动真的在给收益的那一刻亮起（鹰眼校准完成、疾行者移动中、堡垒附近有敌人、破阵者进入末段弹匣、守望者的绝境余生尚未用掉）。
...
- 敌人可掉落弹药、药品和战术道具；武器不走掉落，改为通过关卡阶段奖励确定性交付：每把武器都有固定的解锁关卡与阶段，本局未满 6 把时会直接加入当前编队并永久解锁许可，满编队时只解锁许可供后续在武器库调整。
...
- 第二批六把武器各自引入一种新节奏：M16A4 一次扣扳机打出三连发并真的扣 3 发弹；AA-12 是全自动霰弹；双持乌兹两管同时出弹换取极高射速；特斯拉枪命中后沿附近敌群连跳 4 次，敌人越密越强；磁轨炮按住蓄力 1.1 秒，松手时按蓄力比例结算伤害与穿透；冷冻喷射器用扇形持续减速替代持续伤害。
- 特斯拉枪与磁轨炮使用独立的能量电池弹种，不与步枪弹共享备用弹池。
...
- 七种战术道具都能从对应感染体掉落并由玩家携带布置；地雷/投掷罐负责即时爆破或控场，两种桶和高爆包允许玩家自己决定引爆时机。
- 油桶爆炸后留下持续伤害火焰，面粉桶产生短暂阻挡感染体的粉尘区。
- 油桶、面粉桶和地雷可被其它爆炸触发，形成连锁反应。
- 静态障碍物会阻挡玩家、感染体和子弹。
- 第三关两面危墙可被子弹、玩家爆炸和爆破者轰炸损伤；坍塌会压倒附近感染体并永久打开本局通路，重开后恢复。
- 部分感染体会在明显前摇后冲刺或发射可躲避投射物；四个 Boss 均已接入三阶段、双技能和攻击/死亡动作，其中分段死亡素材会按连续动作播放。
- 10 个固定关卡和无尽模式使用不同地面、边界与环境细节；旋转障碍物使用与视觉对应的碰撞包围盒。
- 第二关的地面、铁轨与边界使用正式位图瓦片（派生自三个 CC0 素材包并统一到该关调色板）；其余关卡与无尽模式仍为程序化绘制，位图缺失时第二关也会自动回退到程序化外观。
...
武器库与怪物图鉴使用固定的方向键 / `WASD` 浏览、`Home` / `End` 跳到首尾、`Esc` 返回主菜单，不受战斗改键影响。武器库另用 `Enter` / 空格编入或移出选中武器，长按不重复切换；未解锁、六武器上限和必带手枪限制仍生效。

> 2026-09-08 档案页公共组件、布局与上述键盘操作已接线，仅做静态核对；新增回归用例、类型检查与浏览器验证尚未执行。范围与待验项见 [`G4-6 执行文档`](docs/execution/2026-09-07-g4-6-archive-components.md)。
...
| `ez:initialWeaponSelectionCompleted` | 是否已完成首次 6 把武器编队确认 |
...
> 历史验证基线（2026-08-25 实测，覆盖当时提交；不覆盖 2026-09-07/08 后续改动）：
>
> | 命令 | 结果 |
> | --- | --- |
...
- 十关原型已具备独立任务简报、主题化环境、障碍、场景物和敌群配置；2026-09-07 已为 11 个主题接入位图三件套与主题叠加层，第三关已接入危墙坍塌试用玩法，逐关完整试玩和长时视觉验收仍缺失。
- 第三关危墙玩法与 120 只敌群已提交；2026-09-07 工作区另有稳定性修复、粒子/UI、环境与教学首批接线。G6-6 五关模板和全关卡剧本/排程门禁已补充。命令层已于 2026-09-15 收口（全量 41 文件 / 673 用例通过、类型检查退出码 0）；浏览器实景已收口三条统一验收用例（U-14 连杀返还、U-17 环境连锁、U-15 模式隔离，证据在 `docs/execution/evidence/`）。构建、其余实景用例、完整试玩与真人判断仍待执行，见 `docs/execution/2026-09-07-unified-acceptance.md`。
