...
export class GameScene extends Phaser.Scene {
  ...
  init(data: GameSceneData): void {
    ...
    this.levelId = data.levelId ?? (this.mode === 'level' ? LEVELS[0]?.id ?? 'level_1' : null);
  }
  ...
  private createBackground(): void {
    this.add.rectangle(GAME_WIDTH / 2, GAME_HEIGHT / 2, GAME_WIDTH, GAME_HEIGHT, 0x1a181c).setDepth(DEPTH.ground);

    const grid = this.add.graphics();
    grid.setDepth(DEPTH.ground);
    grid.lineStyle(1, 0x2a2a2e, 0.65);
  ...
  private updateZombies(_delta: number): void {
    ...
    for (const zombie of zombies) {
      ...
      zombie.seek(this.player.x, this.player.y, separationX, separationY);
    }
  }
  ...
  private spawnEndlessProps(waveNumber: number): void {
    ...
    for (let i = 0; i < spawnCount; i++) {
      ...
      do {
        ...
        attempts += 1;
      } while (attempts < 12 && distanceSq(x, y, this.player.x, this.player.y) < 180 * 180);
  ...
  private applyPickup(pickup: Pickup): boolean {
    ...
    if (drop.type === 'ammo') {
      ...
      const ammoLabel = drop.ammoType === 'heavy' ? '重型弹药' : drop.ammoType === 'shell' ? '霰弹' : '轻型弹药';
