...
export class CreditsScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add.text(GAME_WIDTH / 2, 76, 'CREDITS / LICENSES', { fontFamily: PIXEL_FONT_FAMILY, fontSize: '54px', color: '#f4eedd', stroke: '#455a64', strokeThickness: 6 }).setOrigin(0.5);
    this.add.text(GAME_WIDTH / 2, 180, [
      ...
      '字体：TakWolf · Ark Pixel Font · SIL OFL 1.1',
      '完整来源、许可证、哈希和处理脚本见 docs/ART_ASSET_REGISTRY.md 与 docs/AUDIO_ASSET_REGISTRY.md',
    ].join('\n'), { fontFamily: PIXEL_FONT_FAMILY, fontSize: '19px', lineSpacing: 9, align: 'center', color: '#f4eedd' }).setOrigin(0.5, 0);
    const back = this.add.rectangle(GAME_WIDTH / 2, 640, 280, 48, 0xf4eedd).setStrokeStyle(4, 0x0f0e13);
    this.add.text(GAME_WIDTH / 2, 640, '返回主菜单', { fontFamily: PIXEL_FONT_FAMILY, fontSize: '24px', color: '#0f0e13' }).setOrigin(0.5);
