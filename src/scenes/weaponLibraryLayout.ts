 ...
 */
import type { RowGridLayout } from '../ui/rowGrid';
import { ARCHIVE_LAYOUTS, computeArchiveIndexLayout } from '../ui/archiveLayout';

/** 武器索引首行盒体中心 y。上方是 WEAPON INDEX 标题（y=168）。 */
export const WEAPON_INDEX_FIRST_ROW_Y = ARCHIVE_LAYOUTS.weapon.firstRowCenterY;
/** 页脚分隔线 y。武器索引必须完整停在它上方。 */
export const FOOTER_RULE_Y = ARCHIVE_LAYOUTS.weapon.footerRuleY;
...
export function computeWeaponIndexGrid(weaponCount: number): RowGridLayout {
  return computeArchiveIndexLayout('weapon', weaponCount).grid;
}
