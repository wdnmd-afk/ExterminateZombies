...
export class MainMenuScene extends Phaser.Scene {
  ...
  init(data?: {
    developerCheatActivated?: boolean;
    selectedLevelId?: string;
  }): void {
    this.developerCheatBuffer = "";
    this.showDeveloperCheatConfirmation = Boolean(
      data?.developerCheatActivated,
    );
  ...
  create(): void {
    ...
    const savedUnlocked = SaveManager.load<unknown>(SAVE_KEYS.unlockedLevels, [
      firstLevelId,
    ]);
    const knownLevelIds = new Set(LEVELS.map((level) => level.id));
    const unlockedIds = Array.isArray(savedUnlocked)
      ? savedUnlocked.filter(
          (value): value is string =>
            typeof value === "string" && knownLevelIds.has(value),
        )
      : [firstLevelId];
    const unlocked = new Set(unlockedIds);
    unlocked.add(firstLevelId);

    const orderedUnlocked = LEVELS.map((level) => level.id).filter((id) =>
      unlocked.has(id),
    );
    ...
    if (import.meta.env.DEV) {
      ...
      this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => {
        this.input.keyboard?.off(
          "keydown",
          this.handleDeveloperCheatInput,
          this,
        );
      });
    }
  ...
  private createBackdrop(): void {
    ...
    this.add
      .text(GAME_WIDTH - 20, GAME_HEIGHT / 2, "OUTBREAK", {
  ...
  private createHero(
    ...
    const kicker = this.add.text(
      70,
      42,
      "FIELD REPORT  //  SURVIVAL PROTOCOL",
      {
        fontFamily: UI_FONT_FAMILY,
        fontSize: "15px",
        color: "#fbc02d",
    ...
    const copy = this.add.container(0, 0, [
      kicker,
      title,
      englishTitle,
      tagline,
    ...
    const badgeLabel = this.add
      ...
      .setOrigin(0.5);
    this.missionNumberText = this.add
      .text(1094, 103, "01", {
        fontFamily: UI_FONT_FAMILY,
        fontSize: "88px",
        color: "#0f0e13",
  ...
  private createLevelList(unlocked: Set<string>): Phaser.GameObjects.Container {
    ...
    const counter = this.add
      .text(startX + listWidth, 286, `${LEVELS.length} 个任务区域`, {
        fontFamily: UI_FONT_FAMILY,
        fontSize: "14px",
        color: "#77747b",
      })
    ...
    LEVELS.forEach((level, levelIndex) => {
      ...
      const marker = this.add
        .rectangle(-rowWidth / 2, 0, 6, rowHeight, 0xfbc02d)
        .setOrigin(0, 0.5);
      const index = this.add
        .text(-rowWidth / 2 + 18, 0, String(levelIndex + 1).padStart(2, "0"), {
          fontFamily: UI_FONT_FAMILY,
          fontSize: "23px",
          color: "#f4eedd",
        })
  ...
  private createFooter(canResume: boolean): Phaser.GameObjects.Container {
    ...
    const archiveStatus = this.add.text(
      64,
      651,
      canResume
    ...
    const build = this.add
      .text(GAME_WIDTH - 64, 684, "WEB BUILD  //  v0.1.0", {
        fontFamily: UI_FONT_FAMILY,
        fontSize: "11px",
        color: "#4f4d54",
        letterSpacing: 1,
  ...
  private refreshLevelSelection(): void {
    ...
    const selectedLevel =
      selectedIndex >= 0 ? LEVELS[selectedIndex] : LEVELS[0];
  ...
  private handleDeveloperCheatInput(event: KeyboardEvent): void {
    this.developerCheatBuffer = appendDeveloperCheatInput(
      this.developerCheatBuffer,
      event.key,
    );
  ...
  private createDeveloperCheatConfirmation(): void {
    const box = this.add
      .rectangle(GAME_WIDTH / 2, 28, 520, 42, 0x0f0e13, 0.96)
