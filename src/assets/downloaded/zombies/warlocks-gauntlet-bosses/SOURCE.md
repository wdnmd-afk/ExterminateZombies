...
## 当前状态（2026-09-08，基线 `bc75e37`）

四个 Boss 的移动、攻击、死亡与图鉴立绘现均使用项目生成产物，`PreloadScene` 不再加载本包图像。以下保留初次接入用途，不作为当前运行时清单；只要原始资源仍随仓库分发，就继续保留作者、来源与 CC-BY 3.0 原文。
...
初次接入时只加载四张 `move` 帧条，后曾接入攻击 / 死亡动画；当前整包已退役，不能将这段历史说明当成待实施任务。
