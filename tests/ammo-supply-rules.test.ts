...
function snapshot(overrides: Partial<AmmoSupplySnapshot> = {}): AmmoSupplySnapshot {
  return {
    ...
    ownedWeapons: ['pistol'],
    ammoInMag: { pistol: 7 },
    ammoReserve: { light: 0, heavy: 0, shell: 0, explosive: 0, belt: 0, fuel: 0, energy: 0 },
    ...overrides,
  };
}
...
describe('自适应弹药补给', () => {
  ...
  it('多类武器按真实库存计算缺口权重', () => {
    const multiWeaponSnapshot = snapshot({
      ...
      ammoReserve: { light: 0, heavy: 30, shell: 0, explosive: 0, belt: 0, fuel: 0, energy: 0 },
    });
  ...
  it('库存达到两弹匣目标时把触发概率压到 30%', () => {
    const result = resolveAdaptiveAmmoOpportunity(snapshot({
      ...
      ammoReserve: { light: 0, heavy: 0, shell: 12, explosive: 0, belt: 0, fuel: 0, energy: 0 },
    }), 0.5, 0, () => 0.2);
    expect(result.ammoType).toBeNull();
